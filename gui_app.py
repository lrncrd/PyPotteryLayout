...
class LayoutApp(tk.Tk):
    def __init__(self):
        ...
        self.vars = {
            ...
            'add_table_number': tk.BooleanVar(value=True),  # Add table numbering
            'table_start_number': tk.IntVar(value=1),  # Starting table number
            'table_position': tk.StringVar(value="top_left"),  # Position: top_left, top_right, bottom_left, bottom_right
            'table_font_size': tk.IntVar(value=18),  # Table number font size
            'table_prefix': tk.StringVar(value="Tav."),  # Table number prefix
        }
    ...
    def _create_widgets(self):
        ...
        # Create global header at the top
        ...
        top_container.pack(fill="both", expand=True, padx=5, pady=(0, 5))
    ...
    def _show_info_dialog(self):
        ...
        desc_label = ttk.Label(main_frame, 
                              text="Create archaeological artefacts tables effortlessly\nwith intelligent layout algorithms and export options.",
                              font=("Arial", 12), justify="center")
        desc_label.pack(pady=(0, 15))
        
        # Author
        author_label = ttk.Label(main_frame, text="Author: Lorenzo Cardarelli; Enzo Cocca", 
                                font=("Arial", 12, "bold"))
        ...
        github_label = ttk.Label(github_frame, text="GitHub Project:", 
                                font=("Arial", 12))
        github_label.pack()
        
        # Make the GitHub link clickable
        github_link = ttk.Label(github_frame, text="https://github.com/lrncrd/PyPotteryLayout", 
                               font=("Arial", 11, "underline"), foreground="blue", cursor="hand2")
        github_link.pack()
    ...
    def _open_github_link(self):
        ...
        import webbrowser
        webbrowser.open("https://github.com/lrncrd/PyPotteryLayout")
    ...
    def _create_details_widgets(self, parent):
        # Table numbering section
        table_frame = ttk.Labelframe(parent, text="Table Numbering", padding="10")
        table_frame.pack(fill=X, expand=NO, pady=5)
        
        # Enable table numbering checkbox
        ttk.Checkbutton(table_frame, text="Add Table Numbers", variable=self.vars['add_table_number']).pack(anchor=W, pady=2)
        
        # Table numbering settings in a grid
        settings_frame = ttk.Frame(table_frame)
        settings_frame.pack(fill=X, pady=5)
        
        # Starting number
        ttk.Label(settings_frame, text="Start Number:").grid(row=0, column=0, sticky=W, padx=5, pady=2)
        ttk.Spinbox(settings_frame, from_=1, to=999, textvariable=self.vars['table_start_number'], width=10).grid(row=0, column=1, sticky=W, padx=5, pady=2)
        
        # Font size
        ttk.Label(settings_frame, text="Font Size:").grid(row=0, column=2, sticky=W, padx=5, pady=2)
        ttk.Spinbox(settings_frame, from_=8, to=48, textvariable=self.vars['table_font_size'], width=10).grid(row=0, column=3, sticky=W, padx=5, pady=2)
        
        # Position selection
        ttk.Label(settings_frame, text="Position:").grid(row=1, column=0, sticky=W, padx=5, pady=2)
        position_combo = ttk.Combobox(settings_frame, textvariable=self.vars['table_position'], 
                                     values=["top_left", "top_right", "bottom_left", "bottom_right"], 
                                     state='readonly', width=15)
        position_combo.grid(row=1, column=1, columnspan=2, sticky=W, padx=5, pady=2)
        
        # Table prefix
        ttk.Label(settings_frame, text="Prefix:").grid(row=2, column=0, sticky=W, padx=5, pady=2)
        ttk.Entry(settings_frame, textvariable=self.vars['table_prefix'], width=15).grid(row=2, column=1, sticky=W, padx=5, pady=2)
    ...
    def _update_ui_for_mode(self):
        ...
        # Manual mode temporarily disabled
    ...
    def _run_backend_in_thread(self, params):
        try:
            ...
            self.run_button.config(state='normal', text="Export Layout")

    def _open_output_folder(self):
        """Open the output folder in the system file manager."""
        output_file = self.vars['output_file'].get()
        if not output_file:
            messagebox.showwarning("Warning", "No output file specified. Please set an output file first.")
            return
        ...
        try:
            # Open the folder using system-appropriate command
            ...
            messagebox.showerror("Error", f"Could not open folder: {export_dir}")
        except FileNotFoundError:
            messagebox.showerror("Error", "Could not find system file manager.")
        
        self._update_log(f"📁 Opened output folder: {export_dir}", "info")
    ...
    def _create_preview_panel(self, parent):
        ...
        self.preview_canvas.bind("<Configure>", lambda event: self.after_idle(self._update_preview))
    ...
    def _update_preview(self):
        ...
        # Get page dimensions in preview scale
        ...
        self.preview_canvas.update_idletasks()  # Ensure canvas is properly sized
        canvas_actual_width = self.preview_canvas.winfo_width()
        canvas_actual_height = self.preview_canvas.winfo_height()
        
        # Calculate scale to fit entire page in canvas with some padding
        padding = 20  # Padding around the page
        
        # Only calculate new scale if canvas has reasonable dimensions
        if canvas_actual_width > 100 and canvas_actual_height > 100:
            scale_w = (canvas_actual_width - 2 * padding) / page_width
            scale_h = (canvas_actual_height - 2 * padding) / page_height
            preview_scale = min(scale_w, scale_h, 0.4)  # Increased max scale to 0.4 for better visibility
        else:
            # Fallback scale
            preview_scale = 0.2
        
        # Calculate centered page dimensions
        canvas_width = int(page_width * preview_scale)
        canvas_height = int(page_height * preview_scale)
        
        # Center the page in the canvas
        offset_x = (canvas_actual_width - canvas_width) // 2
        offset_y = (canvas_actual_height - canvas_height) // 2
        
        # Ensure minimum offset
        offset_x = max(padding, offset_x)
        offset_y = max(padding, offset_y)

        # Draw page background (centered)
        self.preview_canvas.create_rectangle(
            offset_x, offset_y, 
            offset_x + canvas_width, offset_y + canvas_height,
            fill="white", outline="black", width=2, tags="page_background"
        )
        ...
        if self.vars['show_margin_border'].get() and margin > 0:
            self.preview_canvas.create_rectangle(
                ...
                offset_x + canvas_width - margin, offset_y + canvas_height - margin,
                outline="gray", dash=(5, 5), tags="page_margin"
            )
        ...
        if mode == "grid":
            self._preview_grid_layout(canvas_width, canvas_height, margin, spacing, scale_factor, preview_scale, offset_x, offset_y)
        elif mode == "puzzle":
            self._preview_puzzle_layout(canvas_width, canvas_height, margin, spacing, scale_factor, preview_scale, offset_x, offset_y)
        elif mode == "masonry":
            self._preview_masonry_layout(canvas_width, canvas_height, margin, spacing, scale_factor, preview_scale, offset_x, offset_y)

        # Update scroll region to show the entire content
        ...
        if all_items:
            ...
            padding_extra = 20
            scroll_region = (
                bbox[0] - padding_extra, 
                bbox[1] - padding_extra, 
                bbox[2] + padding_extra, 
                bbox[3] + padding_extra
            )
            self.preview_canvas.configure(scrollregion=scroll_region)
        else:
            # Default scroll region
            self.preview_canvas.configure(scrollregion=(0, 0, canvas_actual_width, canvas_actual_height))

    def _preview_grid_layout(self, canvas_width, canvas_height, margin, spacing, scale_factor, preview_scale, offset_x, offset_y):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                x = offset_x + margin + col * (cell_width + spacing)
                y = offset_y + margin + row * (cell_height + spacing)
    ...
    def _preview_puzzle_layout(self, canvas_width, canvas_height, margin, spacing, scale_factor, preview_scale, offset_x, offset_y):
            ...
            if y + img_height <= canvas_height - margin:
                try:
                    ...
                    self.preview_canvas.create_rectangle(
                        ...
                        offset_x + x + img_width, offset_y + y + img_height,
                    ...
                    self.preview_canvas.create_text(
                        offset_x + x + img_width/2, offset_y + y + img_height/2,
                        text=f"{idx + 1}",
                        font=("Arial", 14)
                    )
    ...
    def _preview_masonry_layout(self, canvas_width, canvas_height, margin, spacing, scale_factor, preview_scale, offset_x, offset_y):
        ...
        for idx, (path, img) in enumerate(self.preview_images):
            ...
            if y + img_height <= canvas_height - margin:
                try:
                    ...
                    self.preview_canvas.create_rectangle(
                        offset_x + x, offset_y + y, 
                        offset_x + x + new_width, offset_y + y + new_height,
                    ...
                    self.preview_canvas.create_rectangle(
                        offset_x + x, offset_y + y, 
                        offset_x + x + col_width, offset_y + y + img_height,
                        fill="lightyellow", outline="orange"
                    )
                    self.preview_canvas.create_text(
                        offset_x + x + col_width/2, offset_y + y + img_height/2,
                        text=f"{idx + 1}",
                        font=("Arial", 14)
                    )
